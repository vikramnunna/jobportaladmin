...
export default function AddJob() {
  ...
  // API's Function----
  const prefeelData = async () => {
    const res = await getApihandler(`/getJobs/${id}`);
    console.log("res----->", res);

    setValue("title", res.data.title);
    setValue("description", res.data.description);
    setValue("salaryRange", res.data.salaryRange);
    setValue("jobLocation", res.data.jobLocation);
  ...
  const submitJob = async (value) => {
    ...
    const result = await putApihandler(`/updateJobDetails/${id}`, data);
